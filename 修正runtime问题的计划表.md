...
| R-004 | 角色删除需要先预览受影响用户，且对仅绑定单一角色的用户阻断删除 | 已修复 | 删除前先查询角色使用情况；允许安全解绑后再删除，若用户没有其他可用角色则提示先重新配置。 |
| R-005 | 后端管理写操作未形成统一审计，部分操作日志依赖手工埋点可能遗漏 | 已修复 | 已在 API 层新增管理端写请求统一审计中间件（排除 `portal` 与 `auth`），自动写入 `dtlms_operation_logs`。 |
