...
// 2026-07-07: Offer 签署卡片 (录取通知书)
const portalOffer = ref<PortalOfferRecord | null>(null)
const offerCardLoading = ref(false)
const offerCardError = ref<string | null>(null)
// 字典 'student_signed_offer_timeout_hours' 的小时数; 默认 24
const offerTimeoutHours = ref<number>(24)
let offerCountdownTimer: number | null = null

async function loadPortalOfferCard() {
  offerCardLoading.value = true
  offerCardError.value = null
  try {
    const [offerResp, dictResp] = await Promise.all([
      fetchPortalOffer(),
      listDictData({ dict_type: 'student_signed_offer_timeout_hours', status: '启用', page_size: 10 }),
    ])
    portalOffer.value = offerResp.data
    const items: DictDataRecord[] = dictResp?.data?.items || []
    const firstValue = items.length > 0 ? Number(items[0].value) : NaN
    if (Number.isFinite(firstValue) && firstValue > 0) {
      offerTimeoutHours.value = firstValue
    } else {
      offerTimeoutHours.value = 24
    }
  } catch (error: any) {
    offerCardError.value = (error && error.message) ? String(error.message) : '加载录取通知书失败'
  } finally {
    offerCardLoading.value = false
  }
}

const offerSchoolName = computed<string>(() => {
  const v = String(portalOffer.value?.admission_offered_school || '').trim()
  return v || '######大学'
})

function parseOfferDate(raw?: string | null): Date | null {
  if (!raw) return null
...
const offerSignedDateText = computed<string>(() => {
  const d = parseOfferDate(portalOffer.value?.accepted_notification_sent_at) || new Date()
  return formatYmd(d)
})

const offerDeadline = computed<Date | null>(() => {
  const base = parseOfferDate(portalOffer.value?.accepted_notification_sent_at) || new Date()
  if (!Number.isFinite(offerTimeoutHours.value) || offerTimeoutHours.value <= 0) {
    return null
  }
  return new Date(base.getTime() + offerTimeoutHours.value * 3600 * 1000)
})

const offerRemainingMs = ref<number>(0)
function recalcOfferRemaining() {
  const deadline = offerDeadline.value
  if (!deadline) {
    offerRemainingMs.value = 0
    return
  }
  offerRemainingMs.value = Math.max(0, deadline.getTime() - Date.now())
}

const offerRemainingText = computed<string>(() => {
  if (!offerDeadline.value) {
    return '确认已超时'
  }
  const ms = offerRemainingMs.value
  if (ms <= 0) {
    return '确认已超时'
  }
...
const isOfferExpired = computed<boolean>(() => {
  if (!offerDeadline.value) return true
...
function startOfferCountdown() {
  ...
  recalcOfferRemaining()
  offerCountdownTimer = window.setInterval(recalcOfferRemaining, 60_000)
}
...
<template>
  <div class="portal-home-page">
    ...
    <main class="portal-home-main">
      ...
      <!--
        2026-07-07: 录取通知书卡片 (/portal/home/offer)
        - 数据源: /portal/offer (candidate_no / admission_offered_school / accepted_notification_sent_at)
...
/* 2026-07-07: /portal/home/offer 录取通知书卡片样式 */
.portal-offer-card {
  margin: 0 0 24px 0;
  background: #ffffff;
  border-radius: 12px;
...
.portal-offer-card__remaining strong {
  font-weight: 600;
}
...
.portal-offer-card__school {
  color: #1f3864;
  font-weight: 700;
  margin: 0 4px;
}

.portal-offer-card__hours {
  color: #1f3864;
  font-weight: 700;
  margin: 0 4px;
}

.portal-offer-card__signature {
  display: flex;
  flex-direction: column;
...
.portal-offer-card__signature-name {
  font-size: 14px;
  color: #303133;
  font-weight: 600;
}

.portal-offer-card__signature-date {
  font-size: 13px;
  color: #606266;
  margin-top: 4px;
}

.portal-offer-card__tip {
  display: flex;
  align-items: center;
  gap: 8px;
  background: #fdf6ec;
