...
function Start-FrontendPreview {
    $command = "& { Set-Location '$FrontendDir'; `n`$host.UI.RawUI.WindowTitle = 'DTLMS Frontend Preview'; `nnpm run preview -- --host $FrontendHost --port $FrontendPort }"
    return Start-Process -FilePath $TerminalExe -ArgumentList @('-NoExit', '-ExecutionPolicy', 'Bypass', '-Command', $command) -PassThru
}
...
Write-Step '启动前端预览服务。'
$frontendProcess = Start-FrontendPreview

if (Wait-PortState -TargetHost $FrontendHost -Port $FrontendPort -DesiredState Listening -TimeoutSeconds 30) {
    Write-Step "前端预览已启动: http://$FrontendHost`:$FrontendPort"
}
else {
    Write-Warning "前端预览启动超时，请查看新打开的前端终端窗口。进程 ID: $($frontendProcess.Id)"
}

Write-Host ''
Write-Host '启动完成。' -ForegroundColor Green
Write-Host "前端预览地址: http://$FrontendHost`:$FrontendPort" -ForegroundColor Green
