...
- 注册学生管理页已新增“模拟学生”能力：仅书院管理员可见，点击后会先生成一次性模拟码，再在新窗口打开学生门户并交换为真实 portal token，后续操作与学生本人登录一致。
...
当前已确认将新增“新闻信息管理”能力，后续开发完成前先在此处暂存需求口径，便于和实现一起对齐：

- 菜单位置：招生管理下新增二级菜单“新闻管理”。
- 系统字典：新增“新闻类型”，字典值为“学生门户通知消息”“学生门户新闻信息”。
- 功能范围：新闻列表的增、删、改、查，编辑器使用富文本并支持图片上传。
- 状态驱动：编辑时仅保留“保存草稿”和“直接发布”两个业务按钮；列表提供单行发布/下线以及多选批量发布/下线。
- 数据模型：新增新闻信息表，至少包含新闻标题、新闻内容、发布者、审核者、发布日期时间、新闻类型字典值字段，并补充必要的管理字段。
- 交付边界：本轮先完成管理后台后端与管理页，学生门户侧展示需求后续再补。
- 更新约定：待新闻功能开发完成后，再回写并精简本 README 中的临时需求说明。
...
注册学生管理页的“模拟学生”动作用于书院管理员代替学生进入门户执行真实操作，当前实现方式如下：

- 在“注册学生管理”列表中点击“模拟学生”后，后端先生成一次性模拟码，并记录一条操作日志。
- 前端在新窗口打开 `/portal?impersonation_code=...`。
- 学生门户页面收到模拟码后，会自动交换为正式 portal token，并跳转到 `/portal/home`。
- 模拟码是一次性、短时有效的，超过有效期后需要重新发起。
- 只有启用状态的注册学生支持模拟登录。
