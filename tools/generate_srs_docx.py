...
ROLE_ROWS = [
    ...
    ('招生管理员', '招生计划、报名申请、资料审核、结果公布与预录取控制'),
    ('书院管理员', '背景评估环节处理、评估意见填写与终局汇总参与'),
...
MODULE_ROWS = [
    ...
    ('招生管理', '招生计划、报名申请、资料审核、背景评估、中心考核、结果公布、预录取', '计划维护、申请检索、评估考核、结果发布与预录取控制'),
    ...
    ('学生门户申请', '门户首页、在线申请 V2 与 8 个申报章节', '自助填报、导师直选、附件上传、草稿保存与提交'),
]
...
PAGE_CATALOG_ROWS = [
    ...
    ('/system/notification-logs', '通知发送日志', '邮件与通知投递结果追踪', 'SystemView section'),
    ...
    ('/portal/application', '申请表 V2', '8 章节导航、长表单、附件、草稿与提交', '门户正式入口'),
    ('/portal/applicationv2', '申请表 V2 兼容路由', '重定向到 /portal/application', '兼容旧链接'),
]
...
OPERATION_MODE_ROWS = [
    ...
    ('门户申请模式', '8 章节导航 + 长表单 + 附件上传 + 草稿保存 + 最终提交', '适用于在线申请 V2 与导师直选报名口径'),
...
BUSINESS_FLOW_ROWS = [
    ...
    ('招生评审', '申请提交并通过资料审核', '按“背景评估 -> 中心考核 -> 结果公布 -> 预录取”顺序推进招生环节', '阶段化招生结论与门户进度状态'),
...
APPROVAL_FLOW_ROWS = [
    ('招生流程', '资料审核 -> 背景评估 -> 中心考核 -> 结果公布 -> 预录取', '招生管理员、书院管理员、导师', '招生工作台、流程中心、门户状态回显'),
...
BUSINESS_RULE_ROWS = [
    ...
    ('BR-007', '在线申请应按 8 个章节组织，报名信息章节采用导师直选模式，第一志愿导师必填。'),
    ('BR-008', '背景评估按多位书院管理员独立提交记录处理，累计 2 票通过进入中心考核，累计 2 票不通过则报名终止。'),
    ('BR-009', '中心考核应按第一志愿和第二志愿分支处理，仅对应志愿导师可提交分数、意见和通过/不通过结论。'),
    ('BR-010', '论文未满足前置条件时不得进入下一学位节点。'),
    ('BR-011', '关键写操作必须记录操作日志，必要时还应记录同步日志或流程轨迹。'),
]
...
FUNCTIONAL_REQUIREMENTS = {
    ...
    '4.3 招生管理': [
        ...
        ('FR-REC-004', '资料审核', '招生管理员', '打开申请详情并执行审核动作', '系统支持资料审核通过、驳回重填，并同步更新申请流转状态。'),
        ('FR-REC-005', '背景评估', '书院管理员', '申请进入待背景评估阶段', '系统支持书院管理员提交评估结论和意见，并按双票终局规则汇总进入中心考核或报名终止。'),
        ('FR-REC-006', '中心考核', '导师', '申请进入第一志愿或第二志愿考核阶段', '系统仅允许对应志愿导师提交考核分数、意见和通过/不通过结论，并按志愿顺序流转。'),
        ('FR-REC-007', '结果公布', '招生管理员', '考核完成或报名终止后进入结果公布阶段', '系统支持形成正式结果并向门户侧回显当前结果与阶段。'),
        ('FR-REC-008', '预录取处理', '招生管理员', '结果公布完成并满足预录取条件', '系统支持进入预录取控制链路，并保留后续通知与状态追踪能力。'),
    ],
    '4.4 学生与研究中心管理': [
    ...
    '4.9 学生门户': [
        ...
        ('FR-PORTAL-002', '申请表 V2', '门户考生', '进入 /portal/application 在线申请页', '系统支持 8 个章节导航、长表单、导师直选、附件上传、保存草稿和提交。'),
        ('FR-PORTAL-003', '附件上传', '门户考生', '在申请表中上传附件', '系统校验附件分类、格式、大小并返回可用地址。'),
        ('FR-PORTAL-004', '申请进度与结果查询', '门户考生', '登录后查看首页或申请页', '系统回显当前所处招生环节、报名状态以及后续结果通知信息。'),
    ],
}
...
DATA_SUMMARY_ROWS = [
    ...
    ('招生业务层', '招生计划、招生申请、背景评估、中心考核、结果公布、预录取', '支撑招生阶段业务推进'),
...
ENTITY_ROWS = [
    ...
    ('背景评估记录', '书院管理员对招生申请提交的独立评估记录。', '招生申请、流程任务、用户'),
    ('中心考核记录', '导师对第一志愿或第二志愿考核阶段提交的评分与结论记录。', '招生申请、导师、流程任务'),
...
PROTOTYPE_SPECS = [
    ...
    },
    {
        ...
        'description': '在线申请 V2 采用章节导航与长表单嵌入式布局，当前唯一正式入口为 /portal/application，并按导师直选口径组织报名信息章节。',
    },
    {
