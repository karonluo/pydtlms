...
def _role_payload(role_id: int = 9, role_code: str = "system_operator") -> dict[str, Any]:
    return {
        "id": role_id,
        "role_code": role_code,
        "role_name": "系统运营",
...
def test_readonly_user_is_forbidden_from_system_write_endpoints(monkeypatch, client: TestClient) -> None:
    token_subjects: dict[str, str] = {}
    user_contexts = {
        "ops.reader": {
            "username": "ops.reader",
    ...
    def fake_authenticate_system_user(username: str, password: str) -> dict[str, Any] | None:
        if username == "ops.reader" and password == "ReadOnly@2026":
            return user_contexts[username]
        return None
    ...
    login_response = client.post(
        "/api/v1/auth/token",
        data={"username": "ops.reader", "password": "ReadOnly@2026"},
    )
    access_token = login_response.json()["access_token"]
    headers = {"Authorization": f"Bearer {access_token}"}
    ...
    delete_response = client.delete("/api/v1/system/roles/9", headers=headers)

    assert create_response.status_code == 403
    assert update_response.status_code == 403
    assert delete_response.status_code == 403
    assert create_response.json()["detail"] == "Missing permissions: system:write"
    assert update_response.json()["detail"] == "Missing permissions: system:write"
    assert delete_response.json()["detail"] == "Missing permissions: system:write"


def test_admin_can_create_update_and_delete_roles_with_system_write_access(monkeypatch, client: TestClient) -> None:
    token_subjects: dict[str, str] = {}
    user_contexts = {
        "admin": {
            "username": "admin",
    ...
    _install_login_service(monkeypatch, token_subjects, user_contexts)
    _install_principal_resolution(monkeypatch, token_subjects, user_contexts)
    monkeypatch.setattr("app.api.v1.system.create_role", lambda payload: _role_payload())
    monkeypatch.setattr("app.api.v1.system.update_role", lambda role_id, payload: _role_payload(role_id=role_id))
    monkeypatch.setattr("app.api.v1.system.delete_role", lambda role_id: None)
    ...
    assert create_response.json()["role_code"] == "system_operator"
    assert update_response.status_code == 200
    assert update_response.json()["id"] == 9
